...
const PaymentModal = ({ isOpen, onClose, detail, userid }) => {
  ...
  const updateSellStatusForProduct = async (productid) => {
    console.log(
      "🚀 ~ file: PaymentModal.jsx:47 ~ updateSellStatusForProduct ~ productid:",
      productid
    );
    try {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const changeST = await axios.put(api + "buydetail/" + payid, {
            paymentstatus,
            productstock: updatedProductStock,
          });
          // ส่งอีเมลและรีโหลดหน้าหลังจากทำรายการเสร็จ
          const sendMail = await axios.post(api + "sender", {
            SubJect: detail.createdby,
            Body: `การซื้อ ${detail.productname} ราคา ${detail.productprice}฿ กำลังจัดส่ง `,
            To: item.email,
            Image: detail.productimages,
          });
          console.log("🚀 ~ handleSubmit ~ detail:", detail);
  ...
  return (
    <div className="fixed top-0 left-0 w-screen h-screen bg-black/50 bg-opacity-25 flex justify-center items-center backdrop-blur-sm">
      <div className="w-[60%] h-[80%]  rounded-lg flex flex-col">
        <button
          className="px-5 py-2.5 mr-5 justify-center items-center place-self-end text-white"
          onClick={() => onClose()}
        >
          ปิด X
        </button>
        <div className="bg-white p-2 rounded-lg">
          ...
          <div className="flex-center space-x-10 mt-4 mb-4">
            ...
            <div className="flex flex-col rounded-md">
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ชื่อผู้ซื้อ: {detail.createdby}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ชื่อรายการ: {detail.productname}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ราคา: {detail.productprice}฿
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                รหัสการซื้อ: {detail.payid}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                รหัสผู้ขาย: {detail.byid}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                รหัสผู้ซื้อ: {detail.userid}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ประเภทสินค้า: {detail.producttype}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                สถานะการชำระ: {detail.paymentstatus}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ชื่อ: {user.fristnameuser} {user.lastnameuser}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                ที่อยู่: {user.useraddress}
              </span>
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
              ...
              <span className="text-gray-800 text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                วันที่ซื้อ:{" "}
                {moment(detail.createddate).locale("th").format("lll")}
              </span>
            </div>
          </div>
          <hr />
          <div className="flex-center mt-2">
            {
              detail.paymentstatus === "กำลังจัดส่ง" ? (
                <button className="black_btn mr-4" onClick={() => onClose()}>
                  ปิดหน้าต่าง
                </button>
              ) : (
                <button
                  ...
                  onClick={() => handleSubmit(detail.payid, detail.productid)}
                >
                  ยืนยันการตรวจสอบ
                </button>
              ) //เมื่อนำid เข้ามาต้องมาใส่นำฟังก์ชั่นของมัน
            }
          </div>
        </div>
      </div>
