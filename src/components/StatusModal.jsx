...
const StatusModal = ({ status, detail }) => {
  ...
  const handleDelete = async () => {
    axios.delete(api + "sellerdetail/" + detail.userdeid).then((res) => {
      ...
      <div className="relative w-[50%] h-[60%] rounded-lg flex flex-col">
        <div className="bg-white p-2 rounded-lg">
          {status === "ไม่ผ่านการอนุมัติ" && (
            <h1 className="text-center text-2xl">ข้อมูลไม่ถูกต้อง</h1>
          )}
          {status === "รออนุมัติ" && (
            <h1 className="text-center text-2xl">
              คุณได้ทำการลงทะเบียนขายไปแล้ว
            </h1>
          )}

          <div className="flex-center mt-2">
            {status === "ไม่ผ่านการอนุมัติ" && (
              <p>คุณ{status}กรุณาลองใหม่อีกครั้ง</p>
            )}
            ...
            {status === "ไม่ผ่านการอนุมัติ" && (
              ...
              >
                <path
                  fill="#ef4444"
                  fill-rule="evenodd"
                  d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10s-4.477 10-10 10a9.966 9.966 0 0 1-4.262-.951l-4.537.93a1 1 0 0 1-1.18-1.18l.93-4.537A9.965 9.965 0 0 1 2 12Zm8.707-2.707a1 1 0 0 0-1.414 1.414L10.586 12l-1.293 1.293a1 1 0 1 0 1.414 1.414L12 13.414l1.293 1.293a1 1 0 1 0 1.414-1.414L13.414 12l1.293-1.293a1 1 0 1 0-1.414-1.414L12 10.586l-1.293-1.293Z"
                  clip-rule="evenodd"
                />
              </svg>
            )}
          ...
          <div className="flex-center mt-4">
            {status === "ไม่ผ่านการอนุมัติ" && (
              <button className="black_btn" onClick={handleDelete}>
                กลับไปยังหน้าแรก
              </button>
            )}
            {status === "รออนุมัติ" && (
