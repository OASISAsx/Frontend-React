...
const Seller = (userid) => {
  ...
  const accept = item.filter(
    (user) => user.recordstatus === "รออนุมัติ"
  ).length;
  const accepted = item.filter(
    (user) => user.recordstatus === "อนุมัติแล้ว"
  ).length;
  ...
  const loadData = async () => {
    const response = await axios.get(api + "sellerdetails").then(
      (result) => {
        setIsLoaded(true);
        setItem(result.data);
      },
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await axios
          .put(api + `sellerdetail/${userid}`, { recordstatus: "อนุมัติแล้ว" })
          .then(async (res) => {
            await axios
              .put(api + `register/role/${userid}`, { roleId: "seller" })
              .then(async (res) => {
  ...
  const withhold = async (userid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await axios
          .put(api + `sellerdetail/${userid}`, {
            recordstatus: "ไม่ผ่านการอนุมัติ",
          })
          .then((res) => {
            loadData();
            Swal.fire("สำเร็จ!", "ระบบได้ส่งข้อมูลไปยังผู้ใช้แล้ว", "success");
          })
          .catch((err) => {
            console.log("🚀 ~ file: page.jsx:62 ~ res ~ err:", err);
          });
      ...
      <section>
                ...
                <div className="sm:flex sm:items-start">
                  <div className="text-center sm:mt-0 sm:ml-2 sm:text-left">
                    ...
                    {/* <div className="text-sm leading-6 font-medium text-gray-400">
                      เวลาอ้างอิง: {currentTime.toLocaleTimeString()}
                    </div> */}
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 gap-2 -mt-7">
          <div className="flex space-x-2">
            <h2 className="text-2xl mr-6  lg:font-bold tracking-tight dark:text-white xs:text-md xs:font-medium">
              จัดการพนักงาน
            </h2>
            {/* <ExportPdf
                        name={"ผู้ใช้ในระบบ"}
                ...
                {item.map((res, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-7 py-4">{res.userid}</td>
                      <td className="px-2 py-4">{res.firstname}</td>
                      <td className="px-4 py-4">{res.lastname}</td>
                      <td className="px-6 py-4">{res.phone}</td>
                      <td className="px-6 py-4">{res.bankname}</td>
                      <td className="px-6 py-4">{res.bankid}</td>
                      <td className="px-6 py-4">
                        {moment(res.createddate)
                          .locale("th")
                          .format("lll" + " น.")}
                      </td>
                      <td className="px-6 py-4">{res.recordstatus}</td>
                      <td className="px-5 py-4 ml-10">
                        <button
                          onClick={() => {
                            setSeller(res), setShowModal(true);
                          }}
                        >
                          <FaSearch
                        ...
                        {res.recordstatus === "รออนุมัติ" ? (
                          <>
                            <button onClick={() => approve(res.userid)}>
                              <BsCheck
                                className="ml-5 text-white rounded-md bg-emerald-700"
                                size={"25px"}
                              ></BsCheck>
                            </button>
                            <button onClick={() => withhold(res.userid)}>
                              <BiX
                                className="ml-4 text-white rounded-md bg-red-700"
                                size={"25px"}
                              ></BiX>
                            </button>
                          </>
                          ...
                          <button disabled>
                            <BsCheck
                              className="ml-4 text-white rounded-md bg-emerald-700/70"
                              size={"25px"}
                            ></BsCheck>
                          </button>
                        ) : (
                          <button disabled>
                            <BiX
                              className="ml-4 text-white rounded-sm bg-red-700"
                              size={"25px"}
                            />
                          </button>
                        )}
