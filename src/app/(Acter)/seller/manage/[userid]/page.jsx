...
const sellermanage = () => {
  ...
  async function handleChangesST(e, productid) {
    ...
    axios.put(api + "product/" + productid, { status }).then((res) => {
      if (svcid === undefined) {
        loadData();
      } else {
        loadDataByid(svcid);
  ...
  const handleSubmit = async (productid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const changeST = await axios.put(api + "product/" + productid, {
            status,
          });
    ...
    <>
      {session?.user.roleId === "seller" && (
        <div className="max-w-2xl py-2 lg:max-w-none justify-center ">
          <h2 className="text-xl lg:font-bold tracking-tight dark:text-white xs:text-md xs:font-medium py-4">
            จัดการสินค้า
          </h2>
                    ...
                    myInt === item.svcid && (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 ">
                          <td className="flex-auto ">
                            {item.status === "ลงขายสินค้าสำเร็จ" ? (
                              <div className="hover:text-amber-400 flex items-center">
                                <svg
                                  width="17"
                                  height="15"
                                  viewBox="0 0 256 256"
                        ...
                        <td className="px-6 py-4 ">
                          ...
                          <Link href={"/seller/editing/" + item.productid}>
                            {" "}
                            <button
                              type="button"
                  ...
                  >
                    ...
                    <td className=" ">
                      <div className="flex-1 flex ">
                        ...
                        {/* <a href="#" className="font-medium text-blue-600 dark:text-blue-500 mr-10">Edit</a> */}
                        <Link href={"/admin/editing/" + item.productid}>
                          {" "}
                          <button
                            type="button"
                            class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
