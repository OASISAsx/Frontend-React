...
const review = () => {
  ...
  console.log(
    "🚀 ~ file: page.jsx:22 ~ review ~ setUserAvatar:",
    setUserAvatar
  );
  ...
  const loadData = async (productid) => {
    const response = await axios.get(api + "reviews").then(
      (result) => {
        setIsLoaded(true);
        setItem(result.data);
      },
  ...
  const loadUser = async () => {
    const response = await axios.get(api + "registers").then(
      (result) => {
        setIsLoaded(true);
  ...
  const loadTicket = async () => {
    const response = await axios.get(api + "products").then(
      (result) => {
        setIsLoaded(true);
  ...
  const loadAvatar = async () => {
    const response = await axios.get(api + "userdetails").then(
      (result) => {
        setIsLoaded(true);
              ...
              <div className=" container py-9 items-center  justify-between bg-zinc-800/10 rounded-2xl shadow-lg">
                    ...
                    <div key={index}>
                      {userdetail.userid === item.userid && (
                        <div className="flex mr-auto">
                          {userAvatar[index] ? (
                            <img
                              className="shadowavatar w-14 h-14 rounded-full flex object-cover"
                              src={userAvatar[index].Avatar}
                            />
                          ) : (
                            <img
                              className="shadowavatar w-20 h-20 rounded-full flex "
                              src={
                          ...
                          <div className="space-y-1 ml-4 text-lg font-medium dark:text-white">
                            <p className="block text-md text-slate-800 font-semibold blue_gradient">
                              {userdetail.nickname}
                            </p>
                          </div>
                        </div>
                ...
                <div className="mr-10 ">
                  ...
                  <div className="mb-2  text-sm text-gray-500 dark:text-gray-400">
                    <p>
                      ได้ทำการเขียนรีวิวไว้เมื่อวันที่{" "}
                      {moment(item.createddate).locale("th").format("ll")}
                    </p>
                  </div>
                  {ticket.map((res, index) => (
                    <div key={index}>
                      {res.productid === item.productid && (
                        <p className="text-slate-500 text-md dark:text-gray-400">
                          ชื่อรายการ : {res.productname}
                        </p>
                      )}
                    </div>
                  ))}
                  <p className="text-slate-500 text-sm dark:text-gray-400">
                    คำอธิบาย/รีวิว : {item.rvcomment}
                  </p>
                  <button
                    onClick={() => {
                    ...
                    <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                      <div className="ml-5">
                        <img
                          ...
                          className="mx-auto h-60 w-80 object-cover rounded-lg"
                        />
                      </div>
                    </div>
                  </div>
